...
define([
  ...
  function(declare, array, html, lang, query, on, _WidgetsInTemplateMixin,
    ...
    esriRequest, Query, QueryTask, LayerChooserFromMapWithDropbox, Filter, CustomFeaturelayerChooserFromMap, ToggleButton) {
    ...
    return declare([BaseWidget, _WidgetsInTemplateMixin], {
      ...
      _afterFilterApplied: function(layerId){
        ...
        var query = new Query();
        query.where = layerInfo.getFilter()? layerInfo.getFilter(): '1=1';
        query.returnGeometry = true;
        query.outSpatialReference = this.map.spatialReference;
        var qryTask = new QueryTask(layerInfo.getUrl());
        //query extent only firstly
        qryTask.executeForExtent(query, lang.hitch(this, function(result){
          if(result.extent && this._isValidExtent(result.extent)){
            this.map.setExtent(result.extent);
            return;
          }
          //some services don't support query extent, do feature query.
          query = new Query();
          query.where = layerInfo.getFilter()? layerInfo.getFilter(): '1=1';
          query.returnGeometry = true;
          query.outSpatialReference = this.map.spatialReference;
          qryTask = new QueryTask(layerInfo.getUrl());
          qryTask.execute(query).then(lang.hitch(this, function(featureSet){
